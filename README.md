...
- **Blocks Cloud (recommended):** Build output is `build/` after `npm run build`. Set `VITE_*` variables in the deployment environment, then use **Deployment** in Blocks Cloud. See [DEPLOYMENT.md](./DEPLOYMENT.md#selise-blocks-cloud-primary-hosting) for commands, env table, IAM notes, and the Construct pipeline caveat.
- **Azure Static Web Apps:** Manual / fallback instructions are in the same [DEPLOYMENT.md](./DEPLOYMENT.md#azure-static-web-apps-fallback).
