...
const getPublicIamUrl = (path: string) => {
  const cleanPath = path.startsWith('/') ? path : `/${path}`;
  const isLocalDev =
    import.meta.env.DEV &&
  ...
  return isLocalDev ? `/blocks-api${cleanPath}` : getApiUrl(cleanPath);
};
...
const requestPublicIam = async <T>(path: string, init: RequestInit = {}): Promise<T> => {
  const response = await fetch(getPublicIamUrl(path), {
    ...init,
    headers: {
  ...
  if (!response.ok) {
    throw new HttpError(response.status, (body ?? { error: response.statusText }) as any);
  }
  ...
  if (
    ...
    typeof body === 'object' &&
    'isSuccess' in body &&
    (body as { isSuccess?: boolean }).isSuccess === false
  ) {
    throw new HttpError(response.status, body as any);
  }
...
export const signupByEmail = (
  ...
  return requestPublicIam('/identifier/v1/People/Signup', {
    method: 'POST',
    body: JSON.stringify({ ...payload, projectKey }),
  });
};

export const getSignupSettings = async (): Promise<IGetSignUpSettingResponse> => {
  const settings = await requestPublicIam<IGetSignUpSettingResponse>(
    `/idp/v1/Iam/GetSignUpSetting?ProjectKey=${projectKey}`
  );

  return {
    ...settings,
    isEmailPasswordSignUpEnabled:
      settings.isEmailPasswordSignUpEnabled ?? (settings as any).isEmailPasswordSignupEnabled,
    isSSoSignUpEnabled: settings.isSSoSignUpEnabled ?? (settings as any).isSsoSignUpEnabled,
  };
};
