...
import { getDemoNotifications, isLocalDemoToken } from '@/modules/auth/utils/demo-session';
...
export const useGetNotifications = (params: GetNotificationsParams) => {
  ...
  return useQuery({
    ...
    enabled: !isLocalDemo,
    initialData: isLocalDemo ? getDemoNotifications : undefined,
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
...
export const useMarkNotificationAsRead = () => {
  ...
  return useMutation({
    mutationFn: isLocalDemo ? async () => ({ isSuccess: true }) : markNotificationAsRead,
...
export const useMarkAllNotificationAsRead = (options?: { onSuccess?: () => void }) => {
  ...
  return useMutation({
    mutationFn: isLocalDemo ? async () => ({ isSuccess: true }) : markAllNotificationAsRead,
    onSuccess: (data) => {
      queryClient.setQueryData<{ notifications: Notification[] }>(['notifications'], (old) => {
        if (!old) return old;
